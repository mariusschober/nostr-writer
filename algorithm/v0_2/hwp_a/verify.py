...
from .model import domain,evaluate,SCORE_FLOOR
...
def score(bundle: dict,model: dict | None,context: TrustedInputs = TrustedInputs()) -> dict:
    ...
    try:
        ...
        need(model is not None and model['version']=='hwp-a-model/0.2','MODEL_UNAVAILABLE')
        ...
        for did in c.documents:
            ...
            for u in uu:
                ...
                if did not in context.admissible_documents or c.documents[did].gap: reason='CAPTURE_NOT_ADMISSIBLE'
        ...
        for aid in target.live:
            a=c.atoms[aid]
            if not a.requirements<=context.admissible_documents or any(c.documents[d].gap for d in a.requirements):
        ...
        return {'valid':True,'target':target.did,'text':target.text,'offsets':offsets,'scalar_scores':vals,'reasons':reasons,
                'document_score':min(vals,default=SCORE_FLOOR),'units':unit_reports,'origin_types':origin_types,'origin_documents':origin_documents}
    except (InvalidTrace,ValueError,TypeError,KeyError,IndexError,OverflowError,RecursionError) as e:
        return {'valid':False,'reason':str(e),'scalar_scores':[],'document_score':SCORE_FLOOR,'units':{}}


def decide(scored: dict,threshold: int,certification_enabled: bool=False,excluded: list[dict]|None=None) -> dict:
    ...
    try:
        ...
        for i in range(len(vals)):
            ...
            reason=scored['reasons'][i] or ('RELEASE_NOT_APPROVED' if not certification_enabled else 'BELOW_THRESHOLD' if not passed[i] else None)
        ...
        return {'verdict':HUMAN if whole else NOT_PROVABLE,'contribution_verdict':HUMAN if contribution else NOT_PROVABLE,
                'ranges':ranges,'reason':None,'candidate_document_pass':bool(vals) and min(vals)>=threshold,
                'certification_enabled':bool(certification_enabled)}
    except (InvalidTrace,ValueError,TypeError,KeyError,IndexError) as e:
        return dict(base,reason=str(e))


def verify(bundle: dict,model: dict|None=None,threshold: int=0,context: TrustedInputs=TrustedInputs(),excluded=None) -> dict:
    scored=score(bundle,model,context)
    enabled=bool(context.approved_release and context.approved_threshold==threshold and model and model.get('purpose')=='validated-release')
    return decide(scored,threshold,enabled,excluded)
