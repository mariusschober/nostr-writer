...
from hwp_a.features import extract,adequate,FEATURE_NAMES
from hwp_a.model import validate_model,fit,HEADS,evaluate,domain,validate_splits,SCORE_FLOOR
from hwp_a.verify import score,verify,decide,TrustedInputs,snapshot
from hwp_a.dataset import extract_dataset,extract_case,false_mask,risk_record
from hwp_a.ledger import validate_ledger,evaluate_ledger,run_ledger
from hwp_a.release import evaluation_cells
from hwp_a.attacks import adaptive_retiming,retime,staged_transcription
...
class ProvenanceTests(unittest.TestCase):
    ...
    def test_unrelated_later_source_not_retroactive(self):
        ...
        bundle={'version':'hwp-a/0.3','documents':[a,b],'target':'a'};m=permissive_fixture_model(bundle)
...
class EvidenceAndLearningTests(unittest.TestCase):
    def test_schema_179_integer_features(self):
        ...
        published=json.loads((Path(__file__).parents[1]/'artifacts/feature-vector.json').read_text())
...
class FurtherRegressionTests(unittest.TestCase):
    ...
    def test_phase_link_leakage_rejected(self):
        from hwp_a.ledger import phase_separation
...
class SchemaAndReleaseTests(unittest.TestCase):
    ...
    def test_mandatory_release_matrix_not_inferred(self):
        from hwp_a.release import validate_release_matrix
    ...
    def test_wrong_condition_rejected_before_scoring(self):
        from hwp_a.release import run_release
