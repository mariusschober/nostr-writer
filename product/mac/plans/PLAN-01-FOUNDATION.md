...
The complete frozen protocol is directly committed; the replacement bootstrap verifies it without extraction. The original WriterFoundation source/tests were not supplied. As part of this stage, create its Package.swift, implement the six contract areas in mac/Packages/WriterFoundation/README.md, and add meaningful tests from DATA/HWP/NOSTR/FOCUS and contracts/interfaces.md. Record these as new implementation, not recovered original code or the historical 47-test result. M06 remains mandatory. Run the new recovery checker, not the missing historical ten-test suite.
