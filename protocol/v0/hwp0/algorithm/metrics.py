...
from .replay import replay,SPACE,need,num
from .dataset import labels_for
from .verify import score
from .model import SCORE_FLOOR


def evaluate_cases(cases: list[dict],model: dict,contexts: dict,threshold: int,claim_kind: str='fresh-composition') -> dict:
    ...
    for case in cases:
        ...
        result=score(case['bundle'],model,contexts[case['id']],claim_kind); valid=result['valid']
        if not valid:
    ...
    return {'threshold':threshold,'claim_kind':claim_kind,'counts':totals,'attempts':len(cases),
