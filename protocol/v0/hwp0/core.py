"""HWP/0 reference: proof mechanics only, not human-writing detection.
...
def leaf(start_digest: bytes, index: int, salt: bytes, event_bytes: bytes) -> bytes:
    ...
    return digest(b'\x00'+encode(['HWP/0:event',start_digest,index,salt,event_bytes]))
...
def lineage_commitment(salt:bytes,lineage_bytes:bytes) -> bytes:
    ...
    return digest(encode(['HWP/0:lineage',salt,lineage_bytes]))
...
def policy_check(policy:dict) -> None:
    typed(policy,'verification-policy','label releases capture_authorities evaluators quorum modes require_author identity_authorities denied status_as_of protocol purpose')
    valid_ref(policy['protocol']); require(policy['purpose'] in ('production','conformance'),'policy-purpose')
    require(type(policy['label']) is str and 0<len(policy['label'])<=256,'policy-label')
    _ref_list(policy['releases'],0); _ref_list(policy['evaluators'],0,64)
    integer(policy['quorum'],1,64)
    ...
    require(type(policy['capture_authorities']) is list and 0<=len(policy['capture_authorities'])<=64,'capture-authorities')
    prev=None
...
class _MechanicsVerifier:
    ...
    def verify(self,proof_ref:dict,expected_document:bytes | None=None,disclosures:dict | None=None) -> dict:
        ...
        typed(release,'release','algorithm claim artifacts capture_profiles protocol stage validation')
        ...
        for r in statement['captures']:
            end,kid=self.signed(r); typed(end,'capture-end','start count root complete participation document')
            require(end['complete'] is True,'capture-not-complete'); integer(end['count'],2,MAX_EVENTS); octets(end['root'],32)
            start,skid=self.signed(end['start']); typed(start,'capture-start','session profile adapter nonce subject release parents')
            ...
            captures.append({'ref':r,'start':end['start'],'count':end['count'],'root':end['root'],'subject':start['subject'],'document':end['document']})
        require(next(c for c in captures if c['ref']==statement['target_capture'])['document']==statement['document'],'capture-final-document')
        require(next(c for c in captures if c['ref']==statement['target_capture'])['subject']==statement['author'],'target-subject-binding')
        ...
        if statement['mode']=='disclosed-v1' or self.force_recompute:
            ...
            runner=self.runners.get((statement['release']['sha256'], proof['statement']['sha256']))
